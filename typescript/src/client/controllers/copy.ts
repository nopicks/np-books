...
export async function InitCopy(): Promise<void> {
    on("np-books:copyBook", () => {
        const item = globalThis.exports["np-inventory"].getItemsOfType("book", 20, true).reduce((a1, a2) => {
        ...
        globalThis.exports["np-ui"].showContextMenu(item);
    });
    ...
    RegisterUICallback("np-ui:books:copyBook", async (data, cb) => {
        ...
        const info = data.key;
        const item = globalThis.exports["np-inventory"].hasEnoughOfItem("paper", 1, false, true);
        if (!item) {
